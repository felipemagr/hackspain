...
| Notifier | `src/xray/scoring/notify.py`, `src/xray/scoring/rules.py` | built | Slack or SMTP, ledger at `data/marts/_alerts_sent.json`; `CHANNEL=rules` routes each alert by urgency (`info`, `warning`, `critical`), severity and group through the rule book at `data/serving/alert_rules.json`, and sends a rule's own message the month a score crosses the line it drew (`level_above`, `level_below`); the book is written from the chat (`notifier` agent, which asks Slack or email when the request names no channel) or `POST /api/v1/alert-rules` | `make alerts`, `make notify [CHANNEL=rules]` |
