...
| Notifier | `src/xray/scoring/notify.py`, `src/xray/scoring/rules.py` | built | Slack or SMTP, ledger at `data/marts/_alerts_sent.json`; `CHANNEL=rules` routes each alert by urgency (`info`, `warning`, `critical`), severity and group through the rule book at `data/serving/alert_rules.json`, and sends a rule's own message the month a score crosses the line it drew (`level_above`, `level_below`); an email rule carries its address and an `enabled` switch (off sends nothing); the book is written from the chat (`notifier` agent, which asks Slack or email, and the address, when the request leaves them out) or `POST /api/v1/alert-rules`, and edited from the Alerts rail (`web/src/components/Alarms.tsx` over `PATCH`/`DELETE /api/v1/alert-rules/{id}`); `make email-setup` writes the SMTP lines from an address and its app password; the notifier says when email rules exist on a server without them | `make alerts`, `make notify [CHANNEL=rules]`, `make email-setup` |
