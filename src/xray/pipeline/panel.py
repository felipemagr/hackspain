...
`balances.balance` is the same kind of snapshot, but it is dated and the transactions that moved
it are dated too, so `xray.pipeline.cash` rolls it back into a monthly series. This module
consumes that series rather than the snapshot.
"""
...
def build(
    ...
    Args:
        processed_dir: Staging tables, the output of ``xray.pipeline.clean``.
        marts_dir: Mart tables, where ``xray.pipeline.cash`` wrote ``cash_monthly``.
